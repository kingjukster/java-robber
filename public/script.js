...
async function fetchCurrentStats() {
  try {
    ...
    document.getElementById('stat-robber-loot').textContent = stats.totalRobberLoot;
    document.getElementById('stat-robber-goal').textContent = stats.robberGoal;

    // Update progress bar
    const progressPercent = Math.min((stats.totalRobberLoot / stats.robberGoal) * 100, 100);
    document.getElementById('loot-progress').style.width = `${progressPercent}%`;
